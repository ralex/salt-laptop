...
{% set version = salt['pillar.get']('terraform:terraformer:version', salt['cmd.run']('curl -H "Authorization: token '~ github_token ~'" -sL "https://api.github.com/repos/GoogleCloudPlatform/terraformer/releases/latest" | jq -r ".tag_name"', python_shell=True)) %}
...
{% set version = salt['pillar.get']('terraform:terragrunt:version', salt['cmd.run']('curl -H "Authorization: token '~ github_token ~'" -sL "https://api.github.com/repos/gruntwork-io/terragrunt/releases/latest" | jq -r ".tag_name"', python_shell=True)) %}
...
{% set version = salt['pillar.get']('terraform:terraform-docs:version', salt['cmd.run']('curl -H "Authorization: token '~ github_token ~'" -sL "https://api.github.com/repos/terraform-docs/terraform-docs/releases/latest" | jq -r ".tag_name"', python_shell=True)) %}
...
{% set version = salt['pillar.get']('terraform:tfsec:version', salt['cmd.run']('curl -H "Authorization: token '~ github_token ~'" -sL "https://api.github.com/repos/tfsec/tfsec/releases/latest" | jq -r ".tag_name"', python_shell=True)) %}
/usr/local/bin/tfsec:
  file.managed:
